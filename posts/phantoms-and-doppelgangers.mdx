...
## Graph & dependencies introduction
...
### Depends-on in Javascript

- We use the `pacakge.json` and `dependencies` to state that one package depends-on another
- Your project depends on some other packages, which in turn depend on others
- This produces a dependency-graph, which is a DAG.
